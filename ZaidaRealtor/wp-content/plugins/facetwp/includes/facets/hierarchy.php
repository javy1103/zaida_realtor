...
class FacetWP_Facet_Hierarchy
{
    ...
    function render( $params ) {
        ...
        if ( ! empty( $selected_values[0] ) ) {
            ...
            // Which permalink field to use (term_id or slug)
            $permalink = FWP()->helper->get_setting( 'term_permalink' );
            ...
            if ( 'slug' == $permalink ) {
                $slug = esc_sql( $value );
                $facet_parent_id = (int) $wpdb->get_var( "SELECT term_id FROM {$wpdb->terms} WHERE slug = '$slug' LIMIT 1" );
            }
            else {
                $facet_parent_id = (int) $depths[ $value ]['term_id'];
            }
            ...
            for ( $i = 0; $i <= $max_depth; $i++ ) {
                $prev_links[] = array(
                    'value' => $depths[ $last_parent_id ][ $permalink ],
...
(function($) {
    wp.hooks.addAction('facetwp/load/hierarchy', function($this, obj) {
        $this.find('.type-hierarchy .facet-source-taxonomy').val(obj.source);
        $this.find('.type-hierarchy .facet-orderby').val(obj.orderby);
        $this.find('.type-hierarchy .facet-count').val(obj.count);
    });

    wp.hooks.addFilter('facetwp/save/hierarchy', function($this, obj) {
        obj['source'] = $this.find('.type-hierarchy .facet-source-taxonomy').val();
        obj['orderby'] = $this.find('.type-hierarchy .facet-orderby').val();
        obj['count'] = $this.find('.type-hierarchy .facet-count').val();
        return obj;
    });

    wp.hooks.addAction('facetwp/change/hierarchy', function($this) {
...
?>
        <tr class="facetwp-conditional type-hierarchy">
            <td>
            ...
            <td>
                <select class="facet-source-taxonomy">
