(function($){
    ...
    $(document).ready(function() {
        ...
        setTimeout(function(){
            $("[data-value=miami-shores]" ).remove();
        }, 1500);    
                     
                        function loadMapProps(){
                            var mapData = FWP.settings.post_ids;
                            $.ajax({
                                  url: 'http://www.zaidarealtor.com/wp-admin/admin-ajax.php',
                                  type: 'POST',
                                  data: {
                                    'action'  : 'processMapData',
                                    'mapData' : mapData
                                    },
                                  complete: function(xhr, textStatus) {
                                    $(document).scrollTop(250);
                                  },
                                  success: function(data, textStatus, xhr, e) {
                                    window.initializePropertiesMap($.parseJSON(data));
                                    google.maps.event.trigger(window.map, "resize");
                                    console.log("click event triggered and successfull");

                                  },
                                  error: function(xhr, textStatus, errorThrown) {
                                    console.log("click event triggered but failed");
                                    console.log(errorThrown);
                                  }
                            });
                        }
        window.loadMapProps = loadMapProps;
